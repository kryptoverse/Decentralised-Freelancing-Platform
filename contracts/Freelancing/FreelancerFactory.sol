...
contract FreelancerFactory {
    ...
    event CompanyRegistrySet(address indexed registry);
    event VaultLinked(address indexed freelancer, address indexed profile, address indexed vault);
    ...
    // Registry authorized to link company vaults
    ...
    function setCompanyRegistry(address _registry) external onlyOwner {
        if (_registry == address(0)) revert ZeroAddress();
        companyRegistry = _registry;
        emit CompanyRegistrySet(_registry);
    }
    ...
    /// @notice Link a new company vault to a freelancer's profile.
    function linkVaultToProfile(address freelancer, address vaultAddr) external {
        if (msg.sender != companyRegistry) revert NotEscrowDeployerOrOwner(); // Reusing error
        if (freelancer == address(0) || vaultAddr == address(0)) revert ZeroAddress();
        address profileAddr = freelancerProfile[freelancer];
        if (profileAddr == address(0)) revert NoProfile();

        FreelancerProfile(profileAddr).setCompanyVault(vaultAddr);
        emit VaultLinked(freelancer, profileAddr, vaultAddr);
    }
