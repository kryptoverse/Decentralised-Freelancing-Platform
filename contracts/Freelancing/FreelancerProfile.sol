...
contract FreelancerProfile {
    ...
    event CompanyVaultSet(address indexed vault);
    ...
    event EarningsUpdated(uint256 totalEarnings);
    ...
    /*//////////////////////////////////////////////////////////////
                                VAULT (Factory only)
    //////////////////////////////////////////////////////////////*/
    function setCompanyVault(address _vault) external onlyFactory {
        if (_vault == address(0)) revert ZeroAddress();
        if (companyVault != address(0)) revert EscrowAlreadyRegistered(); // Reusing error
        companyVault = _vault;
        emit CompanyVaultSet(_vault);
    }
    ...
    function updateJobStatus(bytes32 jobKey, JobStatus newStatus) external onlyEscrow {
        ...
        // Track active performance stats (disputes/cancellations)
        if (newStatus == JobStatus.Disputed && old != JobStatus.Disputed) {
            disputedJobs++;
        } else if (newStatus == JobStatus.Cancelled && old != JobStatus.Cancelled) {
            cancelledJobs++;
        }
    ...
    /// @notice Records earnings from completed jobs. Called automatically by JobEscrow payout.
    function addEarnings(uint256 amount) external onlyEscrow {
        if (amount == 0) return;
        totalEarnings += amount;
        emit EarningsUpdated(totalEarnings);
    }
