...
interface IJobFundraise {
    function distribute(uint256 amountPaid) external;
    function freelancerPoolTotal() external view returns (uint256);
}

interface ICompanyVaultForEscrow {
    function depositRevenue(uint256 amount) external;
}

interface IEscrowFactoryForDisputes {
    function recordDispute(address client, address freelancer, string calldata reasonURI) external;
    function resolveDisputeRecord(address escrow) external;
}
...
contract JobEscrow is ReentrancyGuard {
    ...
    // FUNDRAISE INTEGRATION
    address public fundraiseContract;
    ...
    event FundraiseLinked(bytes32 indexed jobKey, address indexed fundraiseAddr);

    function setFundraiseContract(address _fundraise) external onlyFreelancer notTerminal {
        if (fundraiseContract != address(0)) revert InvalidCaller();
        if (_fundraise == address(0)) revert ZeroAddress();
        fundraiseContract = _fundraise;
        emit FundraiseLinked(jobKey, _fundraise);
    }
    ...
    function deliverWork(string calldata deliveryURI) external onlyFreelancer notTerminal {
        ...
        // If disputed, we leave the status as Disputed so Admin can review the new submission
        if (!disputed) {
    ...
    function raiseDispute(string calldata reasonURI) external notTerminal {
        ...
        IEscrowFactoryForDisputes(escrowFactory).recordDispute(client, freelancer, reasonURI);
    ...
    function resolveDispute(
        ...
        if (outcome == Resolution.PAYOUT) {
            ...
            if (toFreelancer > 0) {
                ...
                if (netPayout > 0) {
                    if (fundraiseContract != address(0)) {
                        usdt.safeTransfer(fundraiseContract, netPayout);
                        IJobFundraise(fundraiseContract).distribute(netPayout);
                        // Use actual freelancer share (not full payout) for earnings tracking
                        uint256 freelancerNet = IJobFundraise(fundraiseContract).freelancerPoolTotal();
                        profile.addEarnings(freelancerNet);
                    } else {
                        address activeVault = profile.companyVault();
                        if (activeVault != address(0)) {
                            usdt.forceApprove(activeVault, netPayout);
                            ICompanyVaultForEscrow(activeVault).depositRevenue(netPayout);
                        } else {
                            usdt.safeTransfer(freelancer, netPayout);
                        }
                        profile.addEarnings(netPayout);
                    }
                ...
                // NOTE: addEarnings is called within the netPayout > 0 block above (split by fundraise path)
            } else {
            ...
            terminal = true;
            IEscrowFactoryForDisputes(escrowFactory).resolveDisputeRecord(address(this));
    ...
    function _payoutAndComplete(uint8 rating) internal {
        ...
        if (net > 0) {
            if (fundraiseContract != address(0)) {
                usdt.safeTransfer(fundraiseContract, net);
                IJobFundraise(fundraiseContract).distribute(net);
                // Use actual freelancer share (not full net) for earnings tracking
                uint256 freelancerNet = IJobFundraise(fundraiseContract).freelancerPoolTotal();
                profile.addEarnings(freelancerNet);
            } else {
                address activeVault = profile.companyVault();
                if (activeVault != address(0)) {
                    usdt.forceApprove(activeVault, net);
                    ICompanyVaultForEscrow(activeVault).depositRevenue(net);
                } else {
                    usdt.safeTransfer(freelancer, net);
                }
                profile.addEarnings(net);
            }
        ...
        profile.updateJobStatus(jobKey, FreelancerProfile.JobStatus.Approved);
        profile.markJobCompleted(rating, jobKey);
        // NOTE: addEarnings is called within the net > 0 block above (split by fundraise path)
